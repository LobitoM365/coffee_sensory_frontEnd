...
export const Tablas = (array) => {
    ...
    console.log(keysFilterEstado, filterEstado)
    return (
        <>
                        ...
                        <div className="filter-estado">
                            ...
                            <div style={{ display: modalEstado == false ? "none" : "block" }} className="opciones">
                                ...
                                {filterEstado.map((key, index) => {
                                    return <h4 key={key} onClick={() => { changeNameEstadoFocus(key); array.getFilterEstado(keysFilterEstado[key]["value"]) }} className='select-option'>{key}</h4>
                                })}
                                                ...
                                                if (data[valuesD][keys] === "" || data[valuesD][keys] == null || data[valuesD][keys] == undefined) {
                                                    ...
                                                    if (keys == "estado") {
                                                        if (data[valuesD][keys] == 0) {
                                                            return <td key={index}><h4 onClick={() => array.cambiarEstado(data[valuesD]["id"])} className='estado-0'>Inactivo</h4></td>;
                                                        } else {
                                                            return <td key={index}><h4 onClick={() => array.cambiarEstado(data[valuesD]["id"])} className='estado-1'>Activo</h4></td>;
                                                        }
                                                    } else {
